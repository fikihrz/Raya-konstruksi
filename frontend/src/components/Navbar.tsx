...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-surface shadow-md py-3' : 'bg-transparent py-5'}`}>
      <div className="container mx-auto px-4 flex justify-between items-center">
        <Link to="/" className="flex items-center gap-3">
          ...
          <span className={`text-xl font-bold transition-colors ${isScrolled ? 'text-on-surface' : 'text-white'}`}>
            Raya Konstruksi
          </span>
        </Link>
        <div className={`hidden md:flex space-x-6 font-semibold ${isScrolled ? 'text-on-surface' : 'text-white'}`}>
